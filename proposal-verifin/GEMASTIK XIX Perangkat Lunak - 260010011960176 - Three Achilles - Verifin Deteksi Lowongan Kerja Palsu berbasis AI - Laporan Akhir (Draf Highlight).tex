...
\begin{bannerpenyisihan}{Kriteria 2 (Dampak \& Keberlanjutan) -- Protokol Pilot Study \& Valuasi Finansial}
\textbf{Catatan Juri Babak Penyisihan Nasional GEMASTIK XIX (Skor 5/7):} Menjawab evaluasi juri bahwa proyeksi dampak sebelumnya masih berupa simulasi teoretis, belum ada hasil pilot aktual, serta perlunya analisis keberlanjutan biaya komputasi operasional LLM dan mitigasi risiko hukum pencemaran nama baik. Ditambahkan formulasi matematis valuasi ekonomi ($\mathcal{I}_{\text{fin}} = N \cdot p \cdot \eta \cdot \bar{L}$), simulasi 3 skenario adopsi berbasis data BPS dan GASA 2024, rasio Benefit-to-Cost (BCR 1.259:1), rancangan protokol Controlled Pilot Study pada 20 responden (target efisiensi waktu, peningkatan akurasi deteksi, dan instrumen SUS) yang dipersiapkan sebagai roadmap validasi empiris babak final, serta disclaimer hukum mitigasi pencemaran nama baik.
...
\subsubsection*{3.\quad Rancangan Protokol Uji Terbatas Pengguna (Controlled Pilot Study Roadmap)}
Sebagai komitmen menjawab evaluasi dewan juri mengenai pembuktian dampak empiris pada pengguna riil, kami telah menyusun \textbf{Rancangan Protokol Eksperimen Terkontrol (\textit{Controlled Pilot Study Protocol})} yang disiapkan untuk dieksekusi bersama 20 responden representatif (mahasiswa tingkat akhir dan \textit{fresh graduate} di lingkungan kampus):
\begin{enumerate}[label=\textbf{\alph*.}]
\item \textbf{Desain Skenario Kasus Uji:} Pengujian dirancang menggunakan 10 sampel lowongan kerja riil (5 lowongan legitimate dan 5 lowongan penipuan lintas kanal: WhatsApp BUMN palsu, poster luar negeri terindikasi TPPO, formulir phishing, dan portal resmi).
\item \textbf{Target Pengukuran Efisiensi Waktu:} Mengukur durasi investigasi mandiri manual (pencarian Google, peta OSM, cek nomor) dibandingkan verifikasi satu pintu Verifin, dengan target pemangkasan durasi dari baseline $\sim$15--20 menit menjadi $\sim$1--2 menit (target efisiensi waktu $>$90\%).
\item \textbf{Target Efikasi Intervensi Pencegahan:} Mengukur peningkatan akurasi responden dalam mengenali indikator bahaya sebelum dan sesudah membaca narasi transparansi XAI serta Skor Risiko Verifin.
\item \textbf{Evaluasi Kegunaan Antarmuka (SUS):} Menilai tingkat kepuasan dan kemudahan navigasi antarmuka pengguna awam menggunakan instrumen baku \textit{System Usability Scale} (SUS) 10 pertanyaan skala Likert berstandar internasional.
\end{enumerate}
...
\begin{bannerpenyisihan}{Kriteria 1 (Aspek Inovasi) -- Inovasi Integratif Multimodal \& Kritik Ilmiah Dataset EMSCAD}
\textbf{Catatan Juri Babak Penyisihan Nasional GEMASTIK XIX (Skor 5/7):} Menjawab evaluasi dewan juri terkait pembeda kebaruan Verifin terhadap metode deteksi teks tunggal eksisting dan Google Safe Browsing. Disajikan penegasan posisi inovasi integratif multimodal OSINT dalam membongkar modus penipuan di Indonesia, justifikasi ilmiah mengapa model terlatih EMSCAD gagal mendeteksi penipuan lokal (disparitas register percakapan informal dan absennya modus domestik seperti tiket travel BUMN), serta strategi hibrida dua fase: Rule-based NER lokal + LLM evidence-constrained reasoning dilanjutkan fine-tuning IndoBERT-base melalui korpus laporan komunitas.
\end{bannerpenyisihan}
\vspace{0.2cm}
...
% ── SKOR RISIKO TABLE ─────────────────────────────────────────────────────
\begin{bannerpenyisihan}{Kriteria 5 (Kesesuaian Ide \& Perangkat Lunak) -- Standarisasi Ambang Skor Risiko}
\textbf{Catatan Juri Babak Penyisihan Nasional GEMASTIK XIX (Skor 6/7):} Menjawab catatan dewan juri babak penyisihan nasional terkait inkonsistensi ambang batas verdict antara naskah proposal (0--39 / 40--74 / 75--100) dan dokumen teknis (<=25 / 26--60 / >=61). Seluruh subsistem perangkat lunak (pipeline LLM reasoning, formula aditif XAI, dan visualisasi antarmuka) telah diselaraskan penuh pada rentang terkalibrasi: AMAN (0--39), WASPADA (40--74), dan BAHAYA (75--100).
\end{bannerpenyisihan}
\vspace{0.2cm}
...
\begin{bannerpenyisihan}{Kriteria 4 (Proses Rekayasa Perangkat Lunak) -- 146 Test Cases, Code Coverage 88,4\%, \& Ketahanan Asinkron}
\textbf{Catatan Juri Babak Penyisihan Nasional GEMASTIK XIX (Skor 5/7):} Menjawab catatan dewan juri babak penyisihan nasional bahwa pengujian sebelumnya masih menggunakan data kecil terkurasi (126 kasus) dan evaluasi E2E hanya 4 skenario. Disajikan perluasan pengujian menjadi matriks 146 kasus uji otomatis (pass rate 100\%), pengukuran code coverage rata-rata 88,4\% via pytest-cov pada modul inti, penanganan timeout 3,0 detik per probe OSINT, serta mitigasi graceful degradation (status UNAVAILABLE berbobot 0).
\end{bannerpenyisihan}
\vspace{0.2cm}
...
\begin{bannerpenyisihan}{Kriteria 3 \& 5 (Usability UX \& Kepatuhan Privasi) -- Komunitas, Riwayat, \& Protokol SUS}
\textbf{Catatan Juri Babak Penyisihan Nasional GEMASTIK XIX (Skor 5/7 \& 6/7):} Menjawab evaluasi juri terkait pengujian usability aktual, pemahaman pengguna bahwa label AMAN bukan jaminan legalitas, keberlanjutan moderasi laporan komunitas, serta perlindungan privasi (PII). Disajikan rancangan protokol evaluasi System Usability Scale (SUS), audit aksesibilitas WCAG 2.1 AA, antarmuka Community Feed dan Riwayat Verifikasi berbasis Progressive Disclosure, serta penegasan arsitektur sanitasi PII (UU PDP No. 27/2022).
\end{bannerpenyisihan}
\vspace{0.2cm}
