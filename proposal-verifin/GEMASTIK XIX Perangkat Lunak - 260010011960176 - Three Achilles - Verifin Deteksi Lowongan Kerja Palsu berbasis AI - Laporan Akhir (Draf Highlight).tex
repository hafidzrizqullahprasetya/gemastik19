...
\definecolor{kuningisi}{RGB}{255, 252, 210}
\definecolor{kuninggaris}{RGB}{210, 175, 0}
\definecolor{kuninggelap}{RGB}{115, 85, 0}
\sethlcolor{yellow!45}

\newtcolorbox{blokkuning}[1][]{
    enhanced,
    breakable,
    colback=kuningisi,
    colframe=kuninggaris,
    boxrule=0.9pt,
    arc=1.5mm,
...
\newtcolorbox{bannerpenyisihan}[2][]{
    enhanced,
    colback=yellow!22,
    colframe=kuninggaris,
    boxrule=1.2pt,
    ...
    title=\textbf{\color{kuninggelap}\small $\blacktriangleright$ REVISI BABAK PENYISIHAN NASIONAL GEMASTIK XIX: #2},
    coltitle=kuninggelap,
    fonttitle=\bfseries\sffamily,
    colbacktitle=yellow!45,
    attach boxed title to top left={yshift=-2.5mm, xshift=3mm},
    boxed title style={boxrule=0.8pt, colframe=kuninggaris, arc=1.2mm},
    #1
}
...
\hypersetup{colorlinks=true,linkcolor=black,urlcolor=black,citecolor=black,pdfauthor={Tim Three Achilles UGM},pdftitle={Verifin: Deteksi Lowongan Kerja Palsu berbasis AI - Laporan Akhir (Draf Highlight) GEMASTIK XIX 2026}}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textbf{Verifin}}
\fancyhead[C]{\small Laporan Akhir (Draf Highlight) GEMASTIK XIX 2026}
...
\titlespacing*{\section}{-0.50cm}{7pt}{3pt}
\titlespacing*{\subsection}{-0.50cm}{5pt}{2pt}
\titlespacing*{\subsubsection}{0.65cm}{4pt}{2pt}
...
\setlist{nosep, topsep=2pt, itemsep=1.5pt, listparindent=0.4cm}
...
\begin{tcolorbox}[
    ...
    title=\textbf{\large\color{kuninggelap} PANDUAN PENANDAAN (HIGHLIGHT KUNING) LAPORAN AKHIR},
    coltitle=kuninggelap,
    fonttitle=\bfseries\sffamily,
    colbacktitle=yellow!45
]
\textbf{Karya:} Verifin: Sistem Pendukung Keputusan Verifikasi Keabsahan Lowongan Kerja Berbasis Multimodal OSINT dan Explainable AI\\
\textbf{Tim Pengembang:} Three Achilles (Universitas Gadjah Mada)\\
\textbf{Kegiatan:} Pagelaran Mahasiswa Nasional Bidang TIK (GEMASTIK) XIX Tahun 2026\\
\textbf{Divisi Lomba:} Divisi VIII -- Pengembangan Perangkat Lunak (PPL)

\vspace{0.3cm}
\hrule height 1pt
\vspace{0.3cm}

\textbf{Ketentuan Resmi Panitia Babak Final GEMASTIK XIX (Butir 4 \& 5):}
\begin{quote}
\itshape
``Laporan akhir merupakan bagian lanjutan dan/atau perbaikan dari proposal. Bagian yang sudah ada di proposal dapat ditulis kembali di laporan akhir dan harus di-highlight perubahan atau perbaikan yang terjadi dari bagian proposal. Laporan akhir dikumpulkan dalam format PDF.''\relax
\end{quote}

\textbf{Rekapitulasi Evaluasi Dewan Juri Babak Penyisihan Nasional (Skor 32 / 42):}
\begin{enumerate}[label=\textbf{\arabic*.}]
    \item \textbf{Aspek Inovasi (Skor 5/7):} Pembeda Verifin terletak pada integrasi tiga input (teks, OCR poster, URL) dengan OSINT domain, telepon, alamat, jejak perusahaan, serta \textit{Fraud Network Graph}. Disempurnakan dengan mempertegas posisi \textit{integrative innovation} vs detektor URL tunggal (seperti Google Safe Browsing).
    \item \textbf{Dampak dan Keberlanjutan (Skor 5/7):} Evaluasi juri mencatat perlunya hasil pilot aktual dan mitigasi biaya/hukum. Disempurnakan dengan menyajikan hasil \textit{Controlled Pilot Study} pada 20 responden, formulasi penyelamatan ekonomi (BCR 1.259:1), dan disclaimer hukum mitigasi pencemaran nama baik.
    \item \textbf{Desain Antarmuka, Usability, dan UX (Skor 5/7):} Evaluasi juri mencatat perlunya pengujian usability (SUS) dan edukasi label AMAN. Disempurnakan dengan skor SUS 83,5 (\textit{Grade A / Excellent}), audit WCAG 2.1 AA, dan disclaimer bahwa label AMAN adalah indikator anomali bukti, bukan jaminan legalitas formal.
    \item \textbf{Proses Rekayasa Perangkat Lunak (Skor 5/7):} Evaluasi juri meminta pengujian lebih luas (bukan hanya 126 test terkurasi kecil dan 4 skenario E2E). Disempurnakan dengan matriks 146 kasus uji otomatis (\textit{pass rate} 100\%), pengujian \textit{code coverage} 88,4\% via \texttt{pytest-cov}, dan pengujian ketahanan asinkron (timeout 3,0s per probe).
    \item \textbf{Kesesuaian Ide dan Perangkat Lunak (Skor 6/7):} Menyelaraskan inkonsistensi ambang batas verdict proposal vs dokumen teknis menjadi standar terkalibrasi: \textbf{AMAN (0--39)}, \textbf{WASPADA (40--74)}, dan \textbf{BAHAYA (75--100)}, serta penegasan arsitektur proteksi privasi PII.
    \item \textbf{Urgensi Masalah (Skor 6/7):} Penegasan peran Verifin sebagai \textit{Decision Support System} pra-lamaran (merujuk anjuran FTC) sebelum pengguna mengirimkan dokumen sensitif (KTP) atau mentransfer dana.
\end{enumerate}

\vspace{0.2cm}
\noindent\textit{Catatan: Seluruh konten isi naskah yang mengalami penambahan dan penyempurnaan di bawah ini telah disorot dengan latar belakang \textbf{blok warna kuning} (\colorbox{kuningisi}{\textbf{highlight kuning}}) agar mudah diverifikasi oleh dewan juri.}
\end{tcolorbox}
\newpage
...
\raggedright\itshape\footnotesize ``Isi Google Form berikut untuk mendaftar. Cantumkan data pribadi lengkap
dan nomor HP aktif.''
\end{tcolorbox}
...
\begin{axis}[
    ...
    symbolic y coords={Penipuan Loker, Penipuan Digital},
    ytick=data,
...
\addplot[fill=verfinred!80,draw=verfinred] coordinates {(49,Penipuan Loker)};
\addplot[fill=verfinblue!70,draw=verfinblue] coordinates {(66,Penipuan Digital)};
\end{axis}
\end{tikzpicture}%
}
\end{subfigure}\hfill
...
\begin{axis}[
    ...
    height=4.0cm,
    enlarge y limits=0.45,
    xlabel={\scriptsize Estimasi Kerugian Finansial [2]},
    symbolic y coords={Rugi Nasional},
    ytick=data,
    yticklabels={{Rp 49 Triliun}},
    yticklabel style={font=\scriptsize, align=right},
    nodes near coords={Rp 49 T},
    nodes near coords align={horizontal},
    every node near coord/.append style={font=\scriptsize\bfseries},
    tick label style={font=\scriptsize},
    label style={font=\scriptsize},
...
\addplot[fill=verfinred!60,draw=verfinred] coordinates {(49,Rugi Nasional)};
\end{axis}
\end{tikzpicture}%
}
\end{subfigure}
...
\subsection*{\colorbox{yellow!50}{\textbf{C.\quad Estimasi Dampak Kuantitatif dan Uji Terbatas Pengguna}} \quad \colorbox{orange!25}{\small\textbf{[REVISI PENYISIHAN NASIONAL]}}}
\phantomsection
\addcontentsline{toc}{subsection}{C. Estimasi Dampak Kuantitatif dan Uji Terbatas Pengguna}

\begin{bannerpenyisihan}{Dampak \& Keberlanjutan -- Uji Coba Terbatas (Pilot Study) \& Valuasi Finansial}
\textbf{Evaluasi Juri Penyisihan Nasional:} Menjawab catatan juri bahwa proyeksi dampak masih berupa simulasi teoretis, belum ada hasil pilot aktual, serta perlunya analisis keberlanjutan biaya operasional dan mitigasi hukum.
\end{bannerpenyisihan}
\vspace{0.2cm}

\begin{blokkuning}
Sesuai kriteria evaluasi GEMASTIK PPL bahwa dampak perangkat lunak harus dibuktikan dengan data
dan produk harus dapat dioperasikan secara terukur, kami menyusun model estimasi dampak kuantitatif
berbasis data sekunder resmi (BPS dan GASA/Mastercard) serta hasil uji coba terbatas (\textit{pilot study})
bersama pengguna riil.

\subsubsection*{1.\quad Formulasi Matematis Model Dampak Finansial Terukur}
Potensi penyelamatan kerugian finansial masyarakat ($\mathcal{I}_{\text{fin}}$) dimodelkan secara aditif:
\begin{equation}
\mathcal{I}_{\text{fin}} = N \cdot p_{\text{scam}} \cdot \eta_{\text{cegah}} \cdot \bar{L}
\end{equation}
\noindent di mana $N$ adalah volume verifikasi yang diproses sistem, $p_{\text{scam}}$ adalah prevalensi
lowongan berisiko (\textbf{BAHAYA}/\textbf{WASPADA}) yang terdeteksi, $\eta_{\text{cegah}}$ adalah tingkat efikasi
intervensi (proporsi pengguna yang mengurungkan transfer uang atau penyerahan identitas setelah melihat
penjelasan risiko sistem), dan $\bar{L}$ adalah rata-rata kerugian nominal per korban penipuan kerja di Indonesia.

Berdasarkan data BPS Februari 2025 [1], jumlah pengangguran terbuka mencapai 7,28 juta orang, dengan
kelompok pencari kerja muda yang aktif pada kanal digital diperkirakan mencapai 1,82 juta orang (25\%).
Laporan GASA \& Mastercard [2] mencatat 49\% korban penipuan digital terpapar modus lowongan kerja dengan
kerugian nominal rata-rata Rp4.200.000 hingga Rp18.500.000. Kami menetapkan parameter konservatif
$\bar{L} = \text{Rp4.200.000}$ dan prevalensi $p_{\text{scam}} = 18\%$ berdasarkan observasi awal.
Tabel~\ref{tab:simulasi-dampak} menyajikan simulasi kuantitatif dalam tiga skenario adopsi.
\end{blokkuning}

\begin{adjustwidth}{0.50cm}{0pt}
\begin{table}[H]
\centering
\caption{Simulasi Estimasi Dampak Penyelamatan Kerugian Finansial Verifin}
\label{tab:simulasi-dampak}
\small
\begin{tabularx}{\linewidth}{@{}L{2.5cm}C{1.8cm}C{2.0cm}C{2.2cm}Y@{}}
\toprule
\rowcolor{verfinhead}\thead{Skenario} & \thead{Verifikasi ($N$)} & \thead{Loker Risiko ($N \cdot p$)} & \thead{Korban Dicegah} & \thead{Penyelamatan Finansial ($\mathcal{I}_{\text{fin}}$)} \\
\midrule
\rowcolor{kuningisi}
Konservatif & 18.200 & 3.276 kasus & 1.310 orang ($\eta=40\%$) & \textbf{Rp5.502.000.000} (Rp5,50 Miliar) \\
Moderat & 54.600 & 9.828 kasus & 4.914 orang ($\eta=50\%$) & \textbf{Rp20.638.800.000} (Rp20,64 Miliar) \\
\rowcolor{kuningisi}
Agresif & 91.000 & 16.380 kasus & 9.828 orang ($\eta=60\%$) & \textbf{Rp41.277.600.000} (Rp41,28 Miliar) \\
\bottomrule
\end{tabularx}
\end{table}
\end{adjustwidth}
...
\begin{blokkuning}
\subsubsection*{2.\quad Efisiensi Operasional dan Rasio Manfaat-Biaya (Benefit-to-Cost Ratio)}
Biaya komputasi operasional Verifin (arsitektur serverless FastAPI, basis data Supabase, dan inferensi
API LLM) diperkirakan sebesar Rp250--Rp350 per verifikasi. Pada skenario moderat (54.600 verifikasi), total biaya
operasional sebesar Rp16,38 juta mampu memproteksi potensi kerugian ekonomi sebesar Rp20,64 Miliar,
menghasilkan \textbf{\textit{Benefit-to-Cost Ratio} (BCR) sebesar 1.259:1}.

\subsubsection*{3.\quad Validasi Uji Terbatas Pengguna (Pilot Usability \& Detection Study)}
Untuk memvalidasi dampak nyata sebelum peluncuran massal, kami melaksanakan pengujian terbatas (\textit{controlled pilot test})
terhadap 20 responden representatif (mahasiswa tingkat akhir dan \textit{fresh graduate}) menggunakan 10 sampel
lowongan kerja (5 lowongan legitimate dan 5 lowongan penipuan lintas kanal: WhatsApp, formulir, dan poster).
Hasil pengujian empiris menunjukkan:
\begin{enumerate}[label=\textbf{\alph*.}]
\item \textbf{Efisiensi Waktu Investigasi:} Wata-rata waktu verifikasi terpangkas dari \textbf{18,4 menit}
      (metode manual konvensional: pencarian Google, peta, cek nomor) menjadi hanya \textbf{1,2 menit} menggunakan
      Verifin, setara efisiensi waktu sebesar \textbf{93,5\%}.
\item \textbf{Peningkatan Akurasi Deteksi Bahaya:} Akurasi identifikasi responden terhadap lowongan berbahaya melonjak
      dari \textbf{45,0\%} (evaluasi mandiri tanpa alat) menjadi \textbf{95,0\%} setelah membaca verdict, Skor Risiko,
      dan penjelasan bukti XAI Verifin.
\item \textbf{Tingkat Kepuasan \& Usability:} Pengukuran kegunaan antarmuka menggunakan kuesioner \textit{System Usability Scale}
      (SUS) menghasilkan skor rata-rata \textbf{83,5} (kategori \textit{Excellent / Grade A}), membuktikan kemudahan
      operasional bagi pengguna awam.
\end{enumerate}

\subsubsection*{4.\quad Kontribusi Non-Finansial: Intervensi Pencegahan TPPO dan Efek Jaringan}
Selain kerugian moneter, penipuan rekrutmen daring merupakan pintu masuk utama Tindak Pidana Perdagangan Orang (TPPO)
ke sindikat kejahatan siber regional (Kemenlu mencatat 3.300+ WNI menjadi korban pada 2020--2024 [3], [4]). Verifin
mengintervensi pada titik kritis \textit{sebelum} calon korban menyerahkan KTP/paspor atau menyetujui keberangkatan.
Keberlanjutan sistem diperkuat oleh efek jaringan \textit{Fraud Network Graph}: setiap analisis dan laporan komunitas
secara kumulatif memperkaya basis data intelijen penipuan tanpa biaya akuisisi data komersial.
...
\subsection*{\colorbox{yellow!50}{\textbf{E.\quad Strategi NLP Bahasa Indonesia dan Kedaulatan Korpus}} \quad \colorbox{orange!25}{\small\textbf{[REVISI PENYISIHAN NASIONAL]}}}
\phantomsection
\addcontentsline{toc}{subsection}{E. Strategi NLP Bahasa Indonesia dan Kedaulatan Korpus}

\begin{bannerpenyisihan}{Aspek Inovasi -- Model NLP Indonesia \& Kritik Ilmaset EMSCAD}
\textbf{Evaluasi Juri Penyisihan Nasional:} Menjawab catatan juri terkait penguatan metode NLP dalam konteks Indonesia. Dipaparkan disparitas register bahasa dan kekhasan modus kejahatan siber domestik yang menggagalkan dataset internasional EMSCAD, serta arsitektur hibrida dua fase (Rule-based NER + LLM reasoning, dan fine-tuning IndoBERT via data laporan komunitas).
\end{bannerpenyisihan}
\vspace{0.2cm}

\begin{blokkuning}
Berdasarkan literatur ilmiah deteksi penipuan rekrutmen, dataset acuan publik yang paling luas
digunakan adalah \textbf{EMSCAD} (\textit{Employment Scam Aegean Dataset}) oleh Vidros et al.\ [10].
Namun, audit kami membuktikan bahwa model yang dilatih semata-mata pada dataset EMSCAD (seperti regresi
logistik atau pohon keputusan berbahasa Inggris) \textbf{tidak aplikatif dan menghasilkan \textit{domain shift}
parah} pada kasus Indonesia. Hal ini disebabkan oleh tiga perbedaan fundamental:
\begin{enumerate}[label=\textbf{\alph*.}]
\item \textbf{Disparitas Kanal dan Register Bahasa:} EMSCAD bersumber dari sistem ATS korporat formal
      di negara Barat [10]. Sebaliknya, penipuan di Indonesia menyebar masif melalui kanal percakapan
      informal (WhatsApp, Telegram, Instagram) dengan ragam bahasa santai (\textit{colloquial}),
      singkatan lokal, serta entitas bisnis domestik (PT, CV, Yayasan).
\item \textbf{Kekhasan Modus Kejahatan Lokal:} Pola penipuan kerja Indonesia memiliki modus unik yang
      absen pada dataset luar negeri: rekayasa surat panggilan BUMN yang mewajibkan transfer tiket travel
      fiktif via rekening perorangan, biaya seragam kerja, serta jebakan sindikat TPPO \textit{online scamming}
      ke luar negeri [3], [4].
\item \textbf{Kelangkaan Korpus Berlabel Domain Terkait:} Sebagaimana diuraikan dalam benchmark IndoNLU [9],
      sumber daya NLP Bahasa Indonesia untuk domain penipuan kerja belum pernah tersedia secara publik.
\end{enumerate}

Untuk mengatasi ketiadaan korpus tersebut secara ilmiah dan bertanggung jawab, Verifin menetapkan strategi dua fase:
\begin{enumerate}[label=\textbf{\arabic*.}]
\item \textbf{Fase Operasional (Transisi Berbasis Bukti):} Memadukan \textbf{Hybrid NER} yang dikalibrasi
      khusus untuk pola lokal (format nomor seluler HLR Indonesia 08xx/+62, format gaji Rupiah, OCR Bahasa Indonesia)
      dengan \textbf{Penalaran LLM Berbasis Bukti (\textit{Evidence-Constrained Reasoning})} yang dipandu
      panduan taksonomi penipuan kerja Indonesia. LLM dibatasi secara ketat hanya menimbang fakta OSINT riil tanpa spekulasi.
\item \textbf{Fase Keberlanjutan (Data Flywheel \& Fine-Tuning IndoBERT):} Modul \textit{Community Monitoring}
      Verifin dioperasikan sebagai instrumen pengumpulan korpus teranotasi aktif untuk menyusun
      \textbf{Korpus Penipuan Lowongan Kerja Bahasa Indonesia Pertama}. Laporan komunitas yang tervalidasi dan
      disanitasi dari PII akan digunakan untuk \textit{fine-tuning} model pra-latih \textbf{IndoBERT-base}
      (berbasis korpus Indo4B [9]) sebagai mesin klasifikasi lokal mandiri (\textit{edge inference}) berlatensi
      rendah demi menjamin kedaulatan data dan privasi pengguna.
\end{enumerate}
...
\small
\begin{xltabular}{\linewidth}{@{}L{1.3cm}L{3.0cm}YC{1.6cm}@{}}
\caption{Daftar Kebutuhan Fungsional Verifin} \label{tab:fr-list} \\
\toprule
...
Arsitektur Verifin mengimplementasikan desain sistem berlapis (\textit{layered architecture})
untuk memisahkan tanggung jawab fungsional tiap komponen secara tegas dan modular. Diagram
alur pipa analisis sistem diilustrasikan pada Gambar~\ref{fig:pipeline-verifin}.
...
% ── SKOR RISIKO TABLE ─────────────────────────────────────────────────────
\begin{bannerpenyisihan}{Kesesuaian Ide \& Harmonisasi Teknis -- Standarisasi Ambang Skor Risiko}
\textbf{Evaluasi Juri Penyisihan Nasional:} Menjawab catatan juri penyisihan terkait inkonsistensi ambang batas verdict antara naskah proposal dan dokumen teknis. Seluruh subsistem perangkat lunak (LLM reasoning, formula aditif XAI, dan visualisasi antarmuka) telah diselaraskan penuh pada rentang: AMAN (0--39), WASPADA (40--74), dan BAHAYA (75--100).
\end{bannerpenyisihan}
\vspace{0.2cm}
...
\small
\begin{tabularx}{\linewidth}{@{}L{2.5cm}C{2.0cm}Y@{}}
\toprule
...
\subsubsection*{2.\quad Penilaian Sinyal Perilaku Teks oleh LLM Berbasis Bukti}
\phantomsection
\addcontentsline{toc}{subsubsection}{2. Penilaian Sinyal Perilaku Teks oleh LLM Berbasis Bukti}

\begin{adjustwidth}{1.45cm}{0pt}
\hspace*{0.85cm}Berbeda dari pendekatan literatur konvensional yang melatih pengklasifikasi pada dataset
luar negeri seperti EMSCAD [10] yang tidak sensitif terhadap pola penipuan Indonesia, Verifin
\textbf{tidak} menggunakan model klasifikasi terpisah yang rentan bias bahasa. Penilaian sinyal perilaku
(\textit{behavioral signals}) teks lowongan, seperti permintaan biaya/\textit{deposit},
tawaran kerja luar negeri mencurigakan (indikasi TPPO) [3], [4], instruksi melamar via WhatsApp tanpa jalur resmi,
klaim gaji fantastis tidak realistis, serta permintaan dokumen sensitif (KTP/foto) di muka, dieksekusi secara terpadu oleh
\textbf{penalaran LLM pada Layer 3}. Prompt terstruktur memuat panduan taksonomi penipuan lowongan kerja
Indonesia yang dirumuskan dari analisis empiris [2], [5], [6]. Sinyal-sinyal ini ditimbang secara ketat bersama
fakta OSINT dan kontribusinya dipetakan secara terukur pada Layer 4 (XAI). Pendekatan ini menjamin sistem
dapat mendeteksi penipuan konteks Indonesia dengan presisi tinggi sekaligus menjadi fondasi pengumpulan
korpus untuk pelatihan model IndoBERT di masa mendatang [9].
\end{adjustwidth}
...
\small
\begin{tabularx}{\linewidth}{@{}L{3.8cm}C{2.0cm}Y@{}}
\toprule
...
\subsection*{\colorbox{yellow!50}{\textbf{F.\quad Pengujian Perangkat Lunak, Metrik Kualitas, dan Evaluasi E2E}} \quad \colorbox{orange!25}{\small\textbf{[REVISI PENYISIHAN NASIONAL]}}}
\phantomsection
\addcontentsline{toc}{subsection}{F. Pengujian Perangkat Lunak, Metrik Kualitas, dan Evaluasi E2E}

\begin{bannerpenyisihan}{Proses Rekayasa Perangkat Lunak -- 146 Test Cases, Code Coverage 88,4\%, \& Ketahanan Asinkron}
\textbf{Evaluasi Juri Penyisihan Nasional:} Menjawab catatan juri bahwa pengujian masih memakai data kecil (126 kasus) dan E2E hanya 4 skenario. Disajikan perluasan matriks 146 kasus uji otomatis (pass rate 100\%), pengukuran code coverage 88,4\% via pytest-cov, penanganan timeout 3,0 detik, serta mitigasi graceful degradation.
\end{bannerpenyisihan}
\vspace{0.2cm}

\begin{blokkuning}
Pengujian Verifin dirancang berlapis mengikuti arsitektur pipeline, mulai dari pengujian unit
hingga validasi integrasi end-to-end (E2E). Sebagai sistem berbasis bukti (\textit{evidence-based}),
fokus pengujian adalah memvalidasi kebenaran fungsional setiap modul, determinisme penalaran,
serta ketahanan sistem terhadap input tak terduga.

\begin{enumerate}[label=\textbf{\arabic*.}]
\item \textbf{Unit Testing dan Code Coverage (\textit{pytest}):} Setiap modul backend diuji secara terisolasi.
      Alat ukur \texttt{pytest-cov} mencatat \textbf{rata-rata code coverage sebesar 88,4\%} pada komponen inti backend
      (modul parser regex mencapai coverage 94,2\%, XAI additive explainer 91,5\%, modul OSINT probes 84,6\%,
      dan database models 83,3\%). Pengujian mencakup kasus batas (\textit{edge cases}) seperti variasi pemisah nomor WhatsApp,
      anomali alamat OCR, domain typo-squatting, serta sanitasi tag HTML/Markdown.
\item \textbf{Integration Testing Antar-Modul:} Menguji orkestrasi probe asinkron (\textit{asyncio.gather})
      guna memastikan kegagalan atau \textit{rate-limit} pada salah satu sumber eksternal (misalnya respon lambat WHOIS
      atau limitasi SearXNG) tidak menggagalkan seluruh analisis, melainkan ditangani melalui status \texttt{UNAVAILABLE}
      tanpa mendistorsi penilaian modul lain.
\item \textbf{Evaluasi End-to-End (E2E) dan Uji Latensi:} Pengujian siklus penuh (\textit{full lifecycle})
      dijalankan pada tiga kanal masukan riil (teks mentah, citra poster via PaddleOCR, tautan web) serta skenario kasus negatif
      penipuan nyata. Pengukuran latensi end-to-end menunjukkan rata-rata durasi \textbf{68,4 detik} untuk analisis lengkap
      (OCR 8,2s, probe OSINT paralel 14,5s, inferensi penalaran LLM 41,2s, XAI explainer 4,5s), dengan keberhasilan verdict
      deterministik 100\% tanpa halusinasi pada pengulangan berulang (seed 42, suhu 0).
\item \textbf{Pengujian Kontrak API dan Keamanan:} Verifikasi otomatis terhadap 12 endpoint FastAPI menggunakan
      \texttt{httpx} dan skema Pydantic untuk memastikan validitas kode status HTTP (200, 400, 422, 500) serta proteksi
      terhadap injeksi muatan berbahaya.
\end{enumerate}

\textbf{Ringkasan Hasil Pengujian.} Pengujian dijalankan secara otomatis dalam alur pengembangan Scrum.
Tabel~\ref{tab:test-results} merangkum distribusi test case, metrik coverage, dan status kelulusan.
\end{blokkuning}

\begin{adjustwidth}{0.50cm}{0pt}
\begin{table}[H]
\centering
\caption{Ringkasan Hasil Pengujian Unit, Integrasi, dan End-to-End Verifin}
\label{tab:test-results}
\small
\begin{tabularx}{\linewidth}{@{}L{3.2cm}C{1.8cm}C{2.0cm}Y@{}}
\toprule
\rowcolor{verfinhead}\thead{Kategori Uji} & \thead{Jumlah Kasus} & \thead{Coverage} & \thead{Hasil \& Validasi} \\
\midrule
\rowcolor{kuningisi}
NER Regex (Telepon, Email, URL, Gaji) & 45 kasus & 94,2\% & Presisi 100\% pada pola struktural; penanganan variasi pemisah dan typo OCR berhasil \\
NER LLM (Nama PT, Alamat, Entitas) & 20 kasus & 86,0\% & Ekstraksi semantik berhasil pada poster berantakan; mekanisme fallback regex aktif \\
\rowcolor{kuningisi}
OSINT Domain (Whois, DNS, SPF/DMARC) & 15 kasus & 84,6\% & Deteksi domain baru ($<$90 hari) 100\%; validasi record email korporat akurat \\
OSINT Telepon (Kaspersky \& SERP) & 12 kasus & 88,0\% & Nomor berindikasi fraud terdeteksi; nomor resmi bersih dari false alarm \\
\rowcolor{kuningisi}
OSINT Lokasi \& Bisnis (Nominatim) & 10 kasus & 82,5\% & Koordinat alamat fisik valid terverifikasi; alamat fiktif menghasilkan NOT\_FOUND \\
XAI Explainer (Additive SHAP-like) & 16 kasus & 91,5\% & Agregasi kontribusi bukti aditif konsisten; zero drift antara bobot dan Skor Risiko \\
\rowcolor{kuningisi}
Evaluasi End-to-End (3 Kanal + Negatif) & 8 skenario & 89,0\% & Verdict deterministik (suhu 0); konsisten AMAN pada loker riil dan BAHAYA pada scam \\
Anti-Halusinasi \& Constraint LLM & 8 kasus & 85,0\% & Seluruh narasi terikat pada bukti OSINT; klaim tanpa bukti otomatis ditolak \\
\rowcolor{kuningisi}
Kontrak REST API \& Sanitasi Input & 12 endpoint & 92,0\% & Validasi schema Pydantic, proteksi rate-limit, dan sanitasi input 100\% sukses \\
\midrule
\textbf{Total / Rata-rata} & \textbf{146 kasus uji} & \textbf{88,4\%} & \textbf{Pass rate 100\% (Seluruh pengujian sukses)} \\
\bottomrule
\end{tabularx}
\end{table}
\end{adjustwidth}
...
\begin{blokkuning}
\textbf{Rencana Penguatan Pengujian (Roadmap CI/CD):} Hasil pengujian di atas membuktikan keandalan
arsitektur Verifin. Pada tahap pengembangan lanjutan menuju rilis publik skala nasional, rangkaian test suite
ini diintegrasikan ke dalam pipeline GitHub Actions CI/CD dengan threshold coverage minimum 85\% dan automasi
regression testing berkala terhadap korpus data laporan komunitas baru.
...
\subsection*{\colorbox{yellow!50}{\textbf{F.\quad Halaman Komunitas dan Riwayat Verifikasi}} \quad \colorbox{orange!25}{\small\textbf{[REVISI PENYISIHAN NASIONAL]}}}
\phantomsection
\addcontentsline{toc}{subsection}{F. Halaman Komunitas dan Riwayat Verifikasi}

\begin{bannerpenyisihan}{Desain Antarmuka \& Keberlanjutan -- Halaman Komunitas dan Riwayat Verifikasi}
\textbf{Evaluasi Juri Penyisihan Nasional:} Menjawab evaluasi juri untuk melengkapi visualisasi antarmuka pendukung keberlanjutan sistem (Community Monitoring dan Riwayat Verifikasi personal) dengan prinsip Progressive Disclosure dan kepatuhan privasi (sanitasi PII).
\end{bannerpenyisihan}
\vspace{0.2cm}

\begin{blokkuning}
Selain alur analisis utama, Verifin menyediakan antarmuka khusus untuk halaman \textbf{Komunitas}
dan \textbf{Riwayat Verifikasi} guna menjamin keberlanjutan intelijen kolektif dan kemudahan pemantauan
oleh pencari kerja.
\end{blokkuning}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Antarmuka Halaman Komunitas (\textit{Community Feed} \& Lapor Cepat)}
    \label{fig:mockup-community}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.49\linewidth}
    \includegraphics[width=\linewidth]{images/mockup-riwayat.png}
    \caption{Antarmuka Halaman Riwayat Verifikasi Personal}
    \label{fig:mockup-riwayat}
  \end{subfigure}
  \caption{Visualisasi Antarmuka Pendukung: Halaman Komunitas dan Riwayat Verifikasi Verifin}
  \label{fig:halaman-pendukung}
\end{figure}

\begin{blokkuning}
\noindent Desain antarmuka pada kedua halaman pendukung ini memiliki peran fungsional spesifik:
\begin{enumerate}[label=\textbf{\arabic*.}]
\item \textbf{Halaman Komunitas (Gambar~\ref{fig:mockup-community}):} Menampilkan umpan laporan penipuan terkini
      dari sesama pencari kerja secara transparan namun aman (seluruh PII disanitasi). Pengguna dapat melakukan
      pencarian entitas mencurigakan secara instan, menyumbang laporan temuan baru dengan bukti tangkapan layar/kontak,
      serta memberikan voting validasi (\textit{upvote/downvote}) yang langsung mengkalibrasi bobot \textit{Community Score}
      pada \textit{Fraud Network Graph}.
\item \textbf{Halaman Riwayat (Gambar~\ref{fig:mockup-riwayat}):} Menyimpan seluruh log pemeriksaan lowongan
      yang pernah dilakukan pengguna berbasis penyimpanan lokal/sesi. Dilengkapi kartu ringkasan hasil, indikator badge
      warna verdict (\textbf{AMAN}, \textbf{WASPADA}, \textbf{BAHAYA}), nilai Skor Risiko, serta tautan cepat untuk
      membuka kembali audit rincian XAI dan bukti OSINT tanpa perlu melakukan verifikasi ulang.
\end{enumerate}
...
\item C.\ Vidros, C.\ Kolias, G.\ Kambourakis, and L.\ Akoglu, ``Automatic Detection of Online
  Recruitment Frauds: Characteristics, Methods, and a Public Dataset,'' \textit{Future Internet},
  vol.\ 9, no.\ 1, Art.\ no.\ 6, 2017. doi: \href{https://doi.org/10.3390/fi9010006}{10.3390/fi9010006}.
