...
- Berkontribusi pada pencegahan TPPO melalui deteksi dini lowongan pekerjaan palsu yang menjadi pintu masuk perdagangan manusia [3][6].
...
4. **Model Klasifikasi:** Klasifikasi perilaku teks menggunakan TF-IDF + GradientBoostingClassifier yang dilatih pada dataset lowongan berbahasa Indonesia. Model ini bukan model bahasa besar (*large language model*) dan tidak memiliki kemampuan pemahaman semantik mendalam. Peran LLM (Grok via OpenAgentic API) terbatas pada sintesis dan penalaran akhir berdasarkan hasil OSINT yang telah dikumpulkan.
...
- [ ] Implementasi modul klasifikasi NLP: TF-IDF feature extraction + GradientBoostingClassifier untuk deteksi pola teks mencurigakan
...
**Backlog Sprint 2:**
- [ ] Implementasi LLM reasoning layer: integrasi Grok via OpenAgentic API dengan *evidence-only prompting* (LLM hanya menggunakan fakta OSINT terverifikasi)
...
| NLP Classifier | TF-IDF + GradientBoostingClassifier | Klasifikasi perilaku teks berbahasa Indonesia |
| LLM | Grok via OpenAgentic API | Penalaran berbasis bukti, sintesis naratif |
...
│   │   │   ├── nlp_classifier.py      # TF-IDF + GradientBoosting
│   │   │   ├── osint/
│   │   │   │   ├── whois_lookup.py
│   │   │   │   ├── geocoding.py
│   │   │   │   ├── kredibel.py
...
│   │   │   │   └── social_osint.py
│   │   │   ├── llm_reasoning.py       # Grok via OpenAgentic, evidence-only
...
**Interpretasi Verdict:**

| Verdict | Trust Score | Rekomendasi |
|---|---|---|
| AMAN | 70–100 | Lowongan memiliki indikator kepercayaan yang kuat. Lanjutkan proses lamaran dengan tetap berhati-hati. |
| WASPADA | 40–69 | Terdapat beberapa sinyal meragukan. Lakukan verifikasi tambahan sebelum menyerahkan dokumen. |
| BAHAYA | 0–39 | Indikator kuat penipuan terdeteksi. Sangat disarankan untuk tidak merespons lowongan ini. |
