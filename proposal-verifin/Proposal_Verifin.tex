...
\textbf{Verifin (usulan)} & Verifikasi multi-sinyal terintegrasi: NER, aturan teks, OSINT (domain/HP/alamat/form/sosial), penalaran LLM, dan XAI, dalam satu klik & \textbf{Proaktif sebelum respons}, multi-dimensi, berbasis bukti, dapat diaudit (XAI), dan menjangkau kanal informal \\
\bottomrule
\end{tabularx}
\end{table}
...
dan ekonomi kreatif}, yaitu melindungi jutaan pencari kerja dari penipuan lowongan, dan bukan
...
\end{axis}
\end{tikzpicture}
\caption{Distribusi kanal penyebaran lowongan penipuan (GS Lab \& GASA 2024 [2]). WhatsApp dan Instagram mendominasi.}
\label{fig:kanal}
\end{subfigure}\hfill
...
\item Mengimplementasikan pipeline analisis berlapis yang mengintegrasikan Named Entity Recognition
      ...
      berbasis LLM, serta penjelasan kontribusi bukti berbasis XAI hibrida.
...
\item \textbf{Bagi Kemajuan NLP Bahasa Indonesia:} Data laporan komunitas yang terkumpul dari
      ...
      penipuan berbahasa Indonesia. Korpus ini menjawab kelangkaan sumber daya yang selama ini
      menghambat riset NLP Indonesia [9], sekaligus menjadi dasar \textit{fine-tuning} model
      NER/klasifikasi Indonesia di masa depan.
\end{enumerate}
...
pada tahap paling awal, yaitu sebelum korban merespons iklan, berpotensi \textbf{memutus rantai}
...
lain (Inggris, campuran) dapat diterima karena aturan perilaku dan penalaran LLM bersifat toleran
...
\midrule
LLM Orchestration & API LLM (\texttt{kimi-k3}) & Ekstraksi entitas semantik + penalaran verdict berbasis bukti OSINT, deterministik (temperature=0, seed tetap), dengan guard anti-halucination \\
\midrule
...
lokal; dan (ii) penalaran LLM yang secara inheren \textbf{agnostik-bahasa}, yaitu mampu memahami
...
Sistem mengirimkan bundle hasil OSINT (beserta fitur teks dari Layer 2) ke LLM
...
\rowcolor{verfinrow}
\texttt{services/nlp/classifier.py} & Aturan perilaku teks (stub, nonaktif) \\
...
CREATE TABLE jobs (
    ...
    entities JSONB, text_signals JSONB,     -- NER + fitur aturan perilaku
...
\textbf{Layer 1--3: NER, Analisis Perilaku Teks, dan OSINT (ringkas):}

\begin{lstlisting}[language=Python, caption={Output ringkas Layer 1--3 (NER hibrida, aturan perilaku, OSINT)}]
...
// Layer 2 Aturan Perilaku Teks -> label BAHAYA, sinyal teratas:
...
perilaku teks, yaitu deteksi permintaan biaya, dokumen sensitif, gaji fantastis; (c) normalisasi dan
