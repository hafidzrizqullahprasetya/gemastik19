...
\hypersetup{colorlinks=true,linkcolor=black,urlcolor=black,citecolor=black,pdfauthor={Tim Verifin UGM},pdftitle={Proposal GEMASTIK XIX 2026 -- Verifin}}
...
\begin{quote}
\itshape
\textbf{Andri (22), fresh graduate di Yogyakarta.} Menerima broadcast WhatsApp: ``Dibutuhkan Admin
Online WFH, gaji 8--15 juta/bulan, tanpa pengalaman, langsung kerja. Kirim foto KTP \& data diri ke
nomor ini.'' Desakan ekonomi membuat Andri ingin segera merespons. Untuk memverifikasi, ia harus:
mencari nama ``perusahaan'' di Google, mengecek nomor penghubung, memvalidasi alamat kantor, dan
memastikan legalitas --- proses yang tidak ia kuasai dan memakan waktu. Tanpa alat bantu, Andri
berisiko menyerahkan KTP-nya (pintu pencurian identitas) dalam hitungan menit setelah menerima
pesan.
\end{quote}

\begin{quote}
\itshape
\textbf{Sari (27), pencari kerja paruh waktu.} Menemukan poster lowongan menarik di Instagram
dengan tautan pendaftaran Google Form yang meminta data pribadi lengkap dan nomor HP aktif. Sari
tidak tahu bahwa formulir tersebut bukan milik perusahaan resmi, dan tidak ada cara baginya
memastikan apakah jejak digital ``perusahaan'' itu asli atau baru dibuat kemarin.
\end{quote}

Kedua skenario di atas --- yang dihimpun dari pola nyata laporan penipuan lowongan --- menunjukkan
titik kritis yang sama: \textbf{pengguna dipaksa mengambil keputusan sebelum memiliki informasi
yang cukup, dan tidak ada alat yang melakukan verifikasi terintegrasi atas nama mereka.}
Kesenjangan inilah yang menjadi titik intervensi Verifin.
...
teknologi berlapis: Named Entity Recognition (NER) hibrida, aturan perilaku teks, investigasi
OSINT multi-sumber, penalaran naratif LLM, dan penjelasan kontribusi bukti berbasis Explainable
AI (XAI).
...
\textbf{verifikasi proaktif berbasis bukti pada titik penerimaan lowongan} --- celah yang diisi
Verifin.
...
\toprule
\rowcolor{verfingray}
\textbf{Solusi} & \textbf{Cara Kerja} & \textbf{Keterbatasan vs.\ Verifin} \\
\midrule
...
\midrule
\textbf{Fitur ``Lapor'' platform} (WA, IG, Telegram) & Meninjau konten setelah dilaporkan pengguna & \textbf{Reaktif} --- baru berjalan setelah ada korban; tidak memberi penilaian sebelum pengguna merespons \\
\midrule
...
\midrule
\rowcolor{verfinblue!8}
\textbf{Verifin (usulan)} & Verifikasi multi-sinyal terintegrasi: NER + aturan teks + OSINT (domain/HP/alamat/form/sosial) + penalaran LLM + XAI, dalam satu klik & \textbf{Proaktif sebelum respons}, multi-dimensi, berbasis bukti, dapat diaudit (XAI), dan menjangkau kanal informal \\
\bottomrule
\end{tabularx}
\end{table}
...
sosial, form) menjadi satu penilaian holistik yang \emph{dapat dioperasikan pengguna awam} tepat
...
Berkelanjutan menuju Masyarakat Cerdas''}, dengan memanfaatkan kombinasi mutakhir teknologi AI
...
dan ekonomi kreatif} --- melindungi jutaan pencari kerja dari penipuan lowongan --- dan bukan
sekadar demonstrasi teknologi, melainkan perangkat lunak fungsional yang \textbf{dapat
...
\begin{axis}[
  xbar,
  width=10.5cm, height=3.4cm,
  bar width=0.7cm,
  xlabel={Persentase Korban Penipuan Digital (\%)},
  symbolic y coords={Terekspos},
  ytick=data,
  xmin=0, xmax=60,
  nodes near coords,
  nodes near coords align={horizontal},
  title={\textbf{49\% Korban Penipuan Digital Terekspos Modus Lowongan Kerja}},
...
\item Mengimplementasikan pipeline analisis berlapis yang mengintegrasikan Named Entity Recognition
      (NER) hibrida berbahasa Indonesia, aturan perilaku teks, investigasi OSINT multi-sumber
      (domain, nomor telepon, perusahaan, alamat, media sosial), penalaran naratif dan verdict
      berbasis LLM (via OpenAgentic), serta penjelasan kontribusi bukti berbasis XAI hibrida.
...
\item \textbf{Bagi Kemajuan NLP Bahasa Indonesia:} Data laporan komunitas yang terkumpul dari
      ...
      penipuan berbahasa Indonesia --- menjawab kelangkaan sumber daya yang selama ini menghambat
      riset NLP Indonesia [9] --- dan menjadi dasar \textit{fine-tuning} model NER/klasifikasi
...
per korban pada laporan GASA [2], setiap kasus yang berhasil dicegah setara dengan penghindaran
kerugian finansial yang nyata. Sebagai ilustrasi konservatif: pada 1.000 verifikasi dengan
asumsi 20\% terindikasi berisiko dan 30\% pengguna yang diperingatkan memilih tidak melanjutkan,
sekitar \textbf{60 calon korban} terlindungi dalam satu siklus awal --- angka yang akan divalidasi
melalui studi pengguna.

\textbf{(3) Dampak kolektif melalui Fraud Network Graph.} Setiap entitas mencurigakan yang
tercatat memperkuat deteksi bagi pengguna berikutnya (\textit{network effect}). $k$ laporan
komunitas terverifikasi pada satu nomor/akun penipu berpotensi mencegah seluruh calon korban
berikutnya yang akan berinteraksi dengan entitas tersebut, sehingga dampak sistem \textbf{tumbuh
superlinier} terhadap jumlah pengguna, bukan linier.
...
pada tahap paling awal --- sebelum korban merespons iklan --- berpotensi \textbf{memutus rantai}
perdagangan orang pada titik yang paling murah dan paling dapat diotomatisasi oleh perangkat lunak.
...
lain (Inggris, campuran) dapat diterima --- aturan perilaku dan penalaran LLM bersifat toleran
...
\begin{itemize}
  ...
  \item Reputasi nomor telepon Indonesia via \textbf{Kaspersky Who Calls} dan agregasi SERP \textbf{Kredibel}
  \item Validasi alamat fisik dan keberadaan bisnis pada peta via OpenStreetMap (\textbf{Nominatim} dan \textbf{Overpass API})
  \item Pencarian jejak digital perusahaan via metasearch \textbf{SearXNG} multi-engine (DuckDuckGo/Mojeek/Startpage/Brave) dengan filter relevansi entitas, ditunjang peramban ringan Lightpanda sebagai cadangan
...
Pra-penilaian teks menggunakan kombinasi \textbf{aturan perilaku} yang dikalibrasi dari pola
penipuan lowongan kerja Indonesia dan \textbf{penalaran LLM berbasis bukti} yang bersifat
agnostik-bahasa. Keputusan ini diambil secara sadar karena \textbf{dataset berlabel lowongan
...
\midrule
Sprint 1 & Minggu 3--4 & Implementasi pipeline NER hibrida (regex + LLM) + aturan perilaku teks, endpoint FastAPI dasar, UI input form \\
\midrule
...
\rowcolors{2}{verfinrow}{white}
\begin{longtable}{@{}p{1.4cm}p{3.9cm}p{8.3cm}p{1.5cm}@{}}
\toprule
...
\midrule
FR-04 & Analisis Perilaku Teks & Sistem melakukan pra-penilaian teks lowongan menggunakan aturan perilaku yang dikalibrasi dari pola penipuan Indonesia (permintaan biaya, kerja luar negeri, apply-via-WA, gaji fantastis, permintaan dokumen sensitif) & Tinggi \\
\midrule
...
\midrule
FR-08 & Penalaran Naratif \& Verdict LLM & Sistem menghasilkan verdict AMAN/WASPADA/BAHAYA dan ringkasan narasi berbahasa Indonesia menggunakan LLM via OpenAgentic, secara ketat berbasis bukti OSINT (dilarang mengarang fakta di luar evidence) & Tinggi \\
\midrule
...
\midrule
\endhead
NFR-01 & Performa & Pipeline analisis lengkap (OCR $\to$ NER $\to$ Analisis Teks $\to$ OSINT paralel $\to$ LLM reasoning $\to$ XAI) ditargetkan selesai dalam 2--3 menit untuk 90\% permintaan (latensi didominasi sintesis LLM). Modul OSINT dijalankan paralel untuk meminimalkan waktu tunggu, dan pengguna diberi indikator progres bertahap selama analisis berjalan. \\
\midrule
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=4pt, minimum width=1.85cm, minimum height=0.95cm,
              text centered, font=\scriptsize\bfseries, text width=1.75cm},
...
\node[font=\tiny, below=0.15cm of nlp, text=gray] {Aturan Perilaku};
\node[font=\tiny, below=0.15cm of osint, text=gray] {Whois/Phone/Co.};
\node[font=\tiny, below=0.15cm of llm, text=gray] {Verdict berbasis bukti};
...
\node[font=\tiny, align=center] at ($(trust_box.center)+(0,-0.1cm)$)
      {Verdict berbasis bukti \\ OSINT + Teks + Community \\ Anti-halucination Guard \\ AMAN/WASPADA/BAHAYA};
...
\midrule
Analisis Perilaku Teks & Aturan perilaku berbasis domain knowledge (Python) & Deteksi sinyal penipuan Indonesia: permintaan biaya, kerja luar negeri, apply-via-WA, gaji fantastis, dokumen sensitif; agnostik-bahasa, tanpa model terlatih \\
\midrule
LLM Orchestration & OpenAgentic API (\texttt{kimi-k3-high}) & Ekstraksi entitas semantik + penalaran verdict berbasis bukti OSINT, deterministik (temperature=0, seed tetap), dengan guard anti-halucination \\
\midrule
...
\subsubsection*{Layer 2: Analisis Perilaku Teks}

Layer ini melakukan \emph{pra-penilaian} awal teks lowongan menggunakan \textbf{aturan perilaku}
(\textit{behavioral rules}) yang dikalibrasi dari pola penipuan lowongan kerja di Indonesia dan
terinspirasi dari daftar fitur perilaku pada penelitian deteksi penipuan rekrutmen [5], [6].
Aturan ini mengekstrak fitur biner dan numerik seperti: permintaan biaya/\textit{deposit},
tawaran kerja ke luar negeri (indikasi TPPO), instruksi melamar via WhatsApp, gaji fantastis tidak
realistis, permintaan dokumen sensitif (KTP/foto) di awal proses, serta keberadaan identitas
perusahaan. Fitur-fitur ini menjadi sinyal linguistik yang kemudian ditimbang bersama bukti OSINT
pada Layer 4 dan dijelaskan kontribusinya pada Layer 5.

\textbf{Mengapa aturan + LLM, bukan model ML terlatih.} Pemilihan ini adalah keputusan rekayasa
yang disengaja dan berlandaskan temuan ilmiah, bukan keterbatasan teknis. Dataset berlabel
lowongan penipuan \emph{berbahasa Indonesia} hingga saat ini \textbf{belum tersedia secara
publik}; satu-satunya korpus besar yang ada (EMSCAD, 17.880 lowongan) berbahasa Inggris dan tidak
representatif terhadap ragam informal, singkatan, dan bahasa campuran khas iklan lowongan
Indonesia. Kelangkaan sumber daya data berlabel untuk NLP Bahasa Indonesia ini sendiri telah
didokumentasikan sebagai hambatan utama riset pada IndoNLU [9]. Memaksakan model terlatih pada
korpus Inggris berisiko menghasilkan \textit{false confidence} yang menyesatkan pada domain
Indonesia. Oleh karena itu Verifin mengambil pendekatan yang lebih \textbf{jujur dan robust}:
(i) aturan perilaku yang transparan dan dapat diaudit, dikalibrasi langsung dari pola penipuan
lokal; dan (ii) penalaran LLM yang secara inheren \textbf{agnostik-bahasa} --- mampu memahami
konteks lowongan berbahasa Indonesia tanpa memerlukan korpus pelatihan khusus. Ke depan, ketika
dataset Indonesia terkumpul (melalui laporan komunitas Verifin), agenda \textit{fine-tuning}
model klasifikasi/NER Indonesia (mis.\ berbasis IndoBERT [9]) menjadi pengembangan lanjutan yang
terukur.

\textbf{Anti-halucination guard.} Sinyal teks dari layer ini tidak pernah menjadi satu-satunya
dasar verdict. Penalaran LLM pada Layer 4 diinstruksikan secara ketat untuk \textbf{hanya}
menggunakan bukti yang tersedia (temuan OSINT dan fitur teks), dan dilarang mengarang fakta
di luar \textit{evidence}. Setiap sinyal teks diverifikasi silang dengan jejak digital objektif
sebelum berkontribusi pada Skor Risiko.
...
\subsubsection*{Layer 4: LLM Reasoning \& Verdict}

Sistem mengirimkan bundle hasil OSINT (beserta fitur teks dari Layer 2) ke LLM melalui OpenAgentic
...
seluruh bukti dan menghasilkan verdict. Model diinstruksikan untuk:
...
\textbf{Anti-halucination guard.} Agar verdict dapat dipercaya, LLM beroperasi dalam mode
...
Explainer pada Layer 5 sehingga pengguna dapat mengaudit alasan di balik verdict.
...
berpusat pada \textit{evidence explanation}: setiap sinyal bukti --- baik dari aturan perilaku
teks maupun temuan OSINT --- diberi bobot kontribusi yang transparan dan dapat diaudit. Pendekatan
ini dipilih secara sadar: tidak semua penjelasan harus berasal dari metode post-hoc seperti
...
\midrule
\endhead
Analisis Perilaku Teks & 25\% & Skor pra-penilaian dari aturan perilaku (permintaan biaya, kerja luar negeri, apply-via-WA, gaji fantastis, dokumen sensitif, kata kunci penipuan) \\
...
|   |   |- api/v1/
|   |   |   |- verify/          # POST /verify (text/image/url) + pipeline orchestration
|   |   |   |- community/       # GET/POST /community endpoints + moderasi
|   |   |   |- health/          # GET /health + status pipeline
|   |   |- services/
|   |   |   |- ner.py                # Regex NER (entitas struktural)
|   |   |   |- ocr.py                # PaddleOCR wrapper (pre-processing CLAHE)
|   |   |   |- llm/entity_extraction.py  # LLM extraction (entitas semantik)
|   |   |   |- llm/verifin_reasoning.py  # LLM reasoning engine (verdict)
|   |   |   |- nlp/classifier.py     # Aturan perilaku teks (stub, nonaktif)
|   |   |   |- osint/
|   |   |   |   |- runner.py       # Orchestrator probe OSINT paralel
|   |   |   |   |- whois_handler.py # Domain/Whois + keamanan email
|   |   |   |   |- phone_validator.py  # Phone (Kaspersky/Kredibel)
|   |   |   |   |- company_validator.py # Company lookup
|   |   |   |   |- address_validator.py # Alamat (Nominatim/Overpass)
|   |   |   |   |- searxng_client.py   # Metasearch multi-engine
|   |   |   |   |- gform_inspector.py  # Inspeksi Google Form
|   |   |   |   |- social.py        # Penelusuran media sosial
|   |   |   |   |- web_evidence.py  # Agregasi web evidence
|   |   |   |- xai/shap_explainer.py   # SHAP-inspired additive explainer
|   |   |   |- graph/fraud_network.py  # Fraud Network Graph (NetworkX)
|   |   |- database/models.py   # SQLAlchemy JobCase, CommunityReport
...
CREATE TABLE jobs (
    ...
    text_signals  JSONB,                          -- Fitur aturan perilaku teks
    osint_bundle  JSONB,                          -- Hasil seluruh modul OSINT
    llm_summary   TEXT,                           -- Narasi & verdict LLM berbasis bukti
...
\textbf{Layer 2 --- Output Analisis Perilaku Teks:}

\begin{lstlisting}[language=Python, caption={Output JSON Analisis Perilaku Teks Layer 2 (aturan perilaku)}]
{
  "label": "BAHAYA",
  "behavioral_features": {
    "has_fee_request": true,
    "has_whatsapp_apply": true,
    "salary_no_company": true,
    "requests_sensitive_doc": true
  },
  "top_signals": [
    {"feature": "Permintaan Biaya/Transfer", "impact": "risk"},
    {"feature": "Permintaan Dokumen Sensitif (KTP/foto)", "impact": "risk"},
    {"feature": "Gaji Fantastis Tidak Realistis", "impact": "risk"}
  ],
  "method": "Behavioral rules (bahasa-agnostik, dikalibrasi pola penipuan Indonesia)"
}
\end{lstlisting}
...
Analisis Perilaku Teks & $+$24 poin & Aturan perilaku mendeteksi pola penipuan (biaya, dokumen sensitif, gaji fantastis) \\
...
Sinyal Red Flag Teks & $+$13 poin & Permintaan KTP/foto awal + janji langsung kerja (sub-komponen Analisis Perilaku Teks) \\
...
Berbeda dari pendekatan deteksi penipuan konvensional yang mengandalkan satu model klasifikasi
terlatih, Verifin adalah \textbf{sistem berbasis bukti (\textit{evidence-based})}: keputusan
dihasilkan dari agregasi temuan OSINT objektif dan penalaran LLM, sehingga validasinya tidak
...
Namun kami kemudian \textbf{secara sadar tidak menggunakannya dalam sistem produksi} karena korpus
tersebut tidak representatif terhadap ragam bahasa Indonesia --- keputusan yang konsisten dengan
temuan IndoNLU mengenai kelangkaan sumber daya data berlabel Bahasa Indonesia [9]. Kami menilai
lebih bertanggung jawab membangun sistem yang jujur atas keterbatasan datanya daripada mengklaim
akurasi tinggi pada domain yang berbeda. Evaluasi kuantitatif terhadap dataset berlabel Indonesia,
...
\texttt{pytest}: (a) NER --- ekstraksi entitas dari berbagai layout teks/OCR termasuk kasus
tepati (nomor dengan separator, email hasil OCR yang keliru, nama perusahaan ambigu); (b) aturan
perilaku teks --- deteksi permintaan biaya, dokumen sensitif, gaji fantastis; (c) normalisasi dan
validasi nomor telepon Indonesia (whitelist prefix operator, anti false-positive pada ID numerik);
(d) XAI explainer --- konsistensi agregasi kontribusi bukti terhadap Skor Risiko final.
...
menghentikan pipeline --- melainkan menghasilkan status \texttt{UNAVAILABLE} yang jujur pada
komponen tersebut.
...
% TODO: ganti \fbox placeholder ini dengan \includegraphics{images/mockup-community.png}
% setelah tangkapan layar halaman komunitas tersedia.
\begin{figure}[H]
  \centering
  % \includegraphics[width=0.88\linewidth]{images/mockup-community.png}
  \fbox{\parbox[c][6cm][c]{0.85\linewidth}{\centering\itshape Screenshot Halaman Komunitas\\(formulir pelaporan entitas + umpan laporan termoderasi)}}
  \caption{Halaman Komunitas: formulir pelaporan entitas mencurigakan (nama perusahaan, nomor HP, email, atau URL) beserta umpan laporan komunitas yang telah terverifikasi moderator, yang terintegrasi ke dalam Fraud Network Graph.}
  \label{fig:mockup-community}
\end{figure}

% TODO: ganti \fbox placeholder ini dengan \includegraphics{images/mockup-riwayat.png}
% setelah tangkapan layar halaman riwayat tersedia.
\begin{figure}[H]
  \centering
  % \includegraphics[width=0.88\linewidth]{images/mockup-riwayat.png}
  \fbox{\parbox[c][6cm][c]{0.85\linewidth}{\centering\itshape Screenshot Halaman Riwayat Verifikasi\\(daftar verifikasi: badge verdict, Skor Risiko, waktu pemeriksaan)}}
  \caption{Halaman Riwayat Verifikasi: daftar verifikasi yang pernah dilakukan beserta badge verdict, Skor Risiko, dan waktu pemeriksaan, memungkinkan pengguna membandingkan dan merujuk kembali hasil analisis sebelumnya.}
