...
\item Membangun platform web \textbf{Verifin} sebagai \textit{Explainable AI-powered Decision
      ...
      \textbf{AMAN}/\textbf{WASPADA}/\textbf{BAHAYA} dan penjelasan berbasis bukti, sehingga menggantikan proses verifikasi manual
...
\midrule
\rowcolor{verfinrow}
\textit{backend/app/main.py} & \textit{Entry point} FastAPI \\
\textit{backend/app/api/v1/} & Endpoint \textit{/verify} (teks/gambar/URL), \textit{/community}, \textit{/health} \\
\rowcolor{verfinrow}
\textit{services/ner.py}, \textit{ocr.py} & NER regex (entitas struktural) dan pembungkus PaddleOCR (pra-proses CLAHE) \\
\textit{services/llm/} & Ekstraksi entitas semantik dan \textit{reasoning engine} verdict \\
\rowcolor{verfinrow}
\textit{services/nlp/classifier.py} & Placeholder nonaktif (\textit{status: STUB}); penilaian perilaku teks dilakukan oleh reasoning engine LLM, bukan modul ini \\
\textit{services/osint/} & Orkestrasi probe OSINT paralel: Whois/domain, telepon, perusahaan, alamat, SearXNG, Google Form, media sosial, web evidence \\
\rowcolor{verfinrow}
\textit{services/xai/ shap\_explainer.py} & Penjelasan kontribusi bukti aditif (SHAP-inspired) \\
\textit{services/graph/ fraud\_network.py} & Fraud Network Graph (NetworkX) \\
\rowcolor{verfinrow}
\textit{backend/database/ models.py} & Model SQLAlchemy: \textit{JobCase}, \textit{CommunityReport} \\
\textit{frontend/src/app/} & Halaman: utama, analisis, hasil, komunitas \\
\rowcolor{verfinrow}
\textit{frontend/src/ components/} & AnalysisForm, VerdictBadge, TrustScoreGauge, OsintBreakdown, XaiExplainer, FraudNetworkGraph, CommunityFeed \\
\bottomrule
\end{tabularx}
\end{table}
\end{adjustwidth}
...
\begin{enumerate}[label=\textbf{\alph*.}, labelindent=0pt, leftmargin=*]
  \item \textbf{Verdict dan Skor Risiko}. Badge berwarna menampilkan putusan final (\textbf{AMAN} / \textbf{WASPADA} / \textbf{BAHAYA}) disertai Skor Risiko (0--100). Skor ini bukan probabilitas, melainkan indeks risiko berbasis agregasi bukti.
...
\begin{adjustwidth}{1.45cm}{0pt}
\begin{enumerate}[label=\textbf{\alph*.}, labelindent=0pt, leftmargin=*]
  \item Jika verdict \textbf{AMAN}: lowongan memiliki indikator kepercayaan yang terverifikasi. Pengguna tetap dianjurkan untuk memverifikasi secara mandiri sebelum memberikan data pribadi.
  \item Jika verdict \textbf{WASPADA}: terdapat beberapa sinyal yang belum dapat dikonfirmasi. Pengguna disarankan melakukan verifikasi lanjutan sebelum merespons lowongan.
  \item Jika verdict \textbf{BAHAYA}: sistem mendeteksi pola penipuan yang kuat. Pengguna disarankan untuk tidak merespons dan dapat melaporkan ke fitur komunitas Verifin atau ke pihak berwenang.
\end{enumerate}
\end{adjustwidth}
