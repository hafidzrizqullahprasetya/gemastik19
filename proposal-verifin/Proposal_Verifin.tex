...
Verifin menerapkan pendekatan \textbf{penjelasan hibrida (\textit{hybrid explanation})} yang
...
Berikut adalah dokumentasi tangkapan layar antarmuka perangkat lunak Verifin yang diambil langsung dari pengujian sistem secara \textit{end-to-end} pada berbagai skenario dan kanal input nyata (teks, gambar poster via OCR, dan tautan/URL). Pengujian mencakup alur penuh mulai dari penginputan data, pemrosesan bertahap pada modal loading, visualisasi indikator risiko, bukti digital OSINT, hingga rekomendasi tindakan bagi pengguna.

\subsection*{G.1 Modal Progres Analisis Pipeline}
\addcontentsline{toc}{subsection}{G.1 Modal Progres Analisis Pipeline}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\linewidth]{images/mockup-loading.png}
  \caption{Modal progres analisis bertahap secara \textit{real-time} (OCR $\to$ OSINT $\to$ Graph DB $\to$ LLM + XAI) yang menjaga transparansi proses verifikasi kepada pengguna.}
  \label{fig:mockup-loading}
\end{figure}

\subsection*{G.2 Kanal Input 1: Verifikasi Teks Lowongan Kerja}
\addcontentsline{toc}{subsection}{G.2 Kanal Input 1: Verifikasi Teks Lowongan Kerja}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\linewidth]{images/tes text.png}
  \caption{Antarmuka input teks lowongan kerja pada Verifin, memungkinkan pengguna melakukan salin-tempel (\textit{copy-paste}) narasi iklan pekerjaan dari WhatsApp atau media sosial.}
  \label{fig:tes-text}
\end{figure}
...
\subsection*{G.3 Kanal Input 2: Verifikasi Gambar Poster (OCR)}
\addcontentsline{toc}{subsection}{G.3 Kanal Input 2: Verifikasi Gambar Poster (OCR)}
...
\begin{figure}[H]
  ...
  \caption{Hasil verifikasi gambar poster (Bagian Detail): teks hasil ekstraksi PaddleOCR 2.8 berserta pembuktian OSINT terhadap entitas yang berhasil terekstraksi.}
...
\subsection*{G.4 Kanal Input 3: Verifikasi Tautan / URL Web}
\addcontentsline{toc}{subsection}{G.4 Kanal Input 3: Verifikasi Tautan / URL Web}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\linewidth]{images/tes link.png}
  \caption{Antarmuka verifikasi tautan (URL), mendukung inspeksi halaman Instagram, situs berita loker, maupun website perusahaan.}
  \label{fig:tes-link}
\end{figure}
...
\subsection*{G.5 Pengujian Kasus Negatif (Deteksi Lowongan Penipuan / Verdict BAHAYA)}
\addcontentsline{toc}{subsection}{G.5 Pengujian Kasus Negatif (Deteksi Lowongan Penipuan / Verdict BAHAYA)}
